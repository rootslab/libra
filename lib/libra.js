...
exports.Libra = ( function () {
    ...
    lproto.push = function ( ocmd ) {
        ...
        if ( ocmd.isMonitor ) m.on = 1;
             ...
             */
            if ( ocmd.isUnsubscription && ! s.active ) {
                msg = cname + ' command is not allowed if the client is not already in PubSub mode.';
                return sendError( ocmd, msg );
            }
            s.on = 1;
        }
    ...
    lproto.update = function ( scmd, channels ) {
        var me = this
            ...
            , isOK = scmd === 'OK'
            , quit = status.quit[ 0 ]
            , sub = ! isOK && ( scmd.match( /^(p|pun|un)?subscribe$/gi ) !== null )
            , tot = -1
            ;
        if ( isOK ) { log( 'STOOOOOOOOOOOOOOOOOOOOOOO', scmd, channels )
            /*
