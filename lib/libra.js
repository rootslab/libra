...
exports.Libra = ( function () {
    ...
    lproto.push = function ( ocmd ) {
        ...
        else if ( ocmd.isSubscription ) {
            /*
             * NOTE: Redis bug here, execute UNSUBSCRIBE when the client is not in
             * Pubsub mode produces messages, but we would expect an error or a single
             * 1-level nested multibulk reply.
             * Then if PubSub mode is not active, discard the command with an error
             * before sending, it is only a way to circumvent the problem in some cases.
             * For now I don't like to count arguments for (P)UNSUBSCRIBE commands.
             * See http://bit.ly/1kDgoi1.
             */
            if ( ! s.active && ocmd.cmd.match( /(p)?unsubscribe/gi ) !== null ) {
                msg = ocmd.cmd + ' command is not allowed if the client is not already in PubSub mode.';
                return sendError( ocmd, msg );
            } else s.on = 1;
        }
        /*
    ...
    lproto.head = function ( pos ) {
        var me = this
            ...
            , curr = cqueue.get( + pos || 0 )
            ;
        return auth ? auth : ( select  ? select : curr );
    };
    ...
    lproto.update = function ( scmd, channels ) {
        var me = this
            ...
            , quit = status.quit[ 0 ]
            , sub = ! isOK &&
                    ( scmd === 'subscribe' ) || ( scmd === 'unsubscribe' ) ||
                    ( scmd === 'psubscribe' ) || ( scmd === 'punsubscribe' )
            , tot = -1
            ;
        if ( isOK ) {
            /*
